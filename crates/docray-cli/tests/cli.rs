...
#[test]
fn explicit_char_has_v1_4_envelope_and_lossless_hierarchy() {
    dps()
        .arg("extract")
        .arg(testdata("simple.pdf"))
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("\"schema_version\":\"1.4\""))
...
fn lean_defaults_to_element_and_emits_fixed_header_lines() {
    ...
    assert_eq!(lines.next(), Some("#docray element v1.4 pages=1"));
    assert_eq!(
        lines.next(),
        Some(
...
fn pptx_requires_element_granularity_and_supports_lean() {
    ...
    dps()
        ...
        .assert()
        .success()
        .stdout(predicate::str::starts_with("#docray element v1.4 pages=1"));
}
